...
Defines the `RoomLobby.vue` component that renders the room list and room creation modal on the `/rooms` lobby page. Covers the component's mounting contract with `LobbyLive`, animated modal, reactive slug preview, asynchronous slug availability badge, and form integration via `useLiveForm`.
