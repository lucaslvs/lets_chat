...
When running generator tasks, pass `--yes`. Always prefer to use generators as a starting point and modify afterwards.
...
New UI components and pages must be built with Vue via LiveVue — not HEEx. HEEx is for layouts and minimal wrappers only. Existing HEEx templates stay as-is unless explicitly asked to migrate. Consult [tech decisions](.agents/docs/tech-decisions.md) before building any new feature.
